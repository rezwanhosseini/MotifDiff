#!/usr/bin/env python
"""
MotifDiff.py — variant effect prediction via PWM scoring
=========================================================
N / gap bug fix
---------------
...
import pandas as pd
from util import (vcfData, MEME_probNorm, MEME_FABIAN,
                  MCspline_fitting, mc_spline,
...
def write_output_diff(filename, mat, names, index=None):
    if index is None:
        pd.DataFrame(mat, columns=names).to_csv(
            filename, header=True, index=False, sep="\t")
    else:
        pd.DataFrame(mat, columns=names, index=index).to_csv(
            filename, header=True, index=True, sep="\t")
...
@app.command()
def variantdiff(
        genome: str = typer.Option(..., help="fasta file for the genome"),
        motif_file: str = typer.Option(..., "--motif", help="meme file for the motifs"),
        vcf: str = typer.Option(..., help="vcf file"),
        strand: strand_type = typer.Option(
            None, "--strand",
            help="whether you want to look on +/- strand of the sequence (default is +)"),
        ...
        norm_method: norm_type = typer.Option(
            None, "--methodNorm",
            help="what method to use for the normalization (iid/motifbased/mixture)"),
    ...
    if kernel == "PWM":
        if diff_score == "FABIAN":
            ...
            kernels, kernel_mask, kernel_norms = motif.parse(
                motif_file, nuc=nucleotide, strand_specific=strand_specific)
        else:
    ...
    kernel_len = kernels.shape[2]   # padded kernel length (for n_output_positions)
    dinucleotide = (nucleotide == "di")
    # Actual motif lengths (non-zero columns per kernel pair).
    # We use the maximum across all motifs as a conservative contamination radius:
    # a position is masked if any motif's window could overlap an N.
    motif_lens = []
    ...
    print(f"looking on {strand} strand for motif matches on windowsize={windowsize}")
    segments = vcfData(vcf, batch, genome, windowsize,
                       dinucleotide=dinucleotide, strand=strand)
    ...
    for i in range(len(segments)):
        ...
        # vcfData.__getitem__ now returns raw sequences as the 5th and 6th elements
        matref, maskref, matalt, maskalt, raw_ref_seqs, raw_alt_seqs = segments[i]

        ref = F.conv1d(matref, kernels)
        alt = F.conv1d(matalt, kernels)

        # ── N / gap masking (bug fix) ──────────────────────────────────────────
        # Set positions contaminated by N or '-' to -inf so they are ignored by
        # max-pool and never reach mc_spline with a spuriously high score.
        _mask_batch(ref, raw_ref_seqs, kernel_len, dinucleotide, motif_len=max_motif_len)
        _mask_batch(alt, raw_alt_seqs, kernel_len, dinucleotide, motif_len=max_motif_len)
        # ──────────────────────────────────────────────────────────────────────
        ...
        if diff_score == "probNorm":
            if mode == "average":
                ...
                # Replace -inf (masked) with 0 AFTER spline so they don't affect avg
                ref = torch.nan_to_num(ref, nan=0.0, posinf=0.0, neginf=0.0)
                alt = torch.nan_to_num(alt, nan=0.0, posinf=0.0, neginf=0.0)
            ...
            if mode == "max":
                ref = ref.view(ref.shape[0], ref.shape[1] // 2, ref.shape[2] * 2)
                alt = alt.view(alt.shape[0], alt.shape[1] // 2, alt.shape[2] * 2)
                # Replace -inf with a very negative finite value so max-pool
                # still works correctly (max of all-inf would be -inf, not 0)
                ref = torch.nan_to_num(ref, nan=-1e9, posinf=-1e9, neginf=-1e9)
                alt = torch.nan_to_num(alt, nan=-1e9, posinf=-1e9, neginf=-1e9)
                _, ref_max_pos = torch.max(torch.tensor(ref[:, :, :windowsize]), dim=2)
                _, alt_max_pos = torch.max(torch.tensor(alt[:, :, :windowsize]), dim=2)
    ...
    write_output_diff(out_file + f"_{nucleotide}_{diff_score}_{mode}.alt",
                      outAlt, motif_names, segments.names())
    write_output_diff(out_file + f"_{nucleotide}_{diff_score}_{mode}.ref",
                      outRef, motif_names, segments.names())
    write_output_diff(out_file + f"_{nucleotide}_{diff_score}_{mode}.diff",
                      outDiff, motif_names, segments.names())
    write_output_diff(out_file + f"_{nucleotide}_{diff_score}.refMaxPos",
                      refMaxPos, motif_names, segments.names())
    write_output_diff(out_file + f"_{nucleotide}_{diff_score}.altMaxPos",
                      altMaxPos, motif_names, segments.names())
